...
/// Includes OS clipboard integration so Ctrl+C/V/X work across apps.
/// </summary>
internal static unsafe class RlImGui
{
    ...
    // ── OS clipboard bridge ───────────────────────────────────────────────────
    // We sync manually: on Ctrl+V we feed the OS clipboard into ImGui's
    // internal buffer; on Ctrl+C/X we push ImGui's buffer out to the OS.
    ...
    public static void Setup()
    {
        ...
        // NavEnableKeyboard intentionally omitted: it steals arrow keys from the game.
    ...
    public static void Begin()
    {
        ...
        // ── OS → ImGui clipboard on Ctrl+V ────────────────────────────────────
        // Feed the OS clipboard into ImGui's internal buffer so InputText can paste it.
    ...
    public static void End()
    {
        ...
        // ── ImGui → OS clipboard on Ctrl+C / Ctrl+X ──────────────────────────
        // ImGui updates its internal clipboard when the user copies; we push it
        // back to the OS so the text is available outside the application.
        // Guard: ImGui.GetClipboardText() throws if the internal buffer is null.
        string? clip = null;
        try { clip = ImGui.GetClipboardText(); } catch { /* empty clipboard */ }
