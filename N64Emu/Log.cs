...
static class Log
{
    ...
    public enum Cat { Info, CPU, GPU, RSP, RDP, DMA, IRQ, Error }
    ...
    public static void Add(Cat cat, string text)
    {
        ...
        if (FileLogging)
        {
            _file ??= new StreamWriter(
                Path.Combine(AppContext.BaseDirectory, "n64_debug.log"), false)
                { AutoFlush = false };
    ...
    public static void RSP  (string t) => Add(Cat.RSP,   t);
    public static void RDP  (string t) => Add(Cat.RDP,   t);
