...
namespace PsxEmu {
 class PsxGte {
  ...
  private uint RES1;
  private int MAC0;
  private int MAC1, MAC2, MAC3;
  private ushort IRGB;
  private int LZCS, LZCR;

  private Matrix RT, LM, LRGB;
  private int TRX, TRY, TRZ;
  private int RBK, GBK, BBK;
  private int RFC, GFC, BFC;
  private int OFX, OFY, DQB;
  private ushort H;
  private short ZSF3, ZSF4, DQA;
  private uint FLAG;
  ...
  internal void Execute(uint command) {
   ...
   switch (command & 0x3F) {
    ...
    default: Console.WriteLine($"UNIMPLEMENTED GTE COMMAND {command & 0x3F:x2}"); break;
   }
  ...
  private void MVMVA() {
   uint mxIndex = (currentCommand >> 17) & 0x3;
   uint mvIndex = (currentCommand >> 15) & 0x3;
   uint tvIndex = (currentCommand >> 13) & 0x3;
  ...
  private void GPL() {
   ...
   MAC1 = (int)(setMAC(1, IR[1] * IR[0] + mac1) >> sf);
   MAC2 = (int)(setMAC(2, IR[2] * IR[0] + mac2) >> sf);
   MAC3 = (int)(setMAC(3, IR[3] * IR[0] + mac3) >> sf);
  ...
  internal void WriteData(uint fs, uint v) {
   switch (fs) {
    ...
    case 15: SXY[0] = SXY[1]; SXY[1] = SXY[2]; SXY[2].val = v; break;
