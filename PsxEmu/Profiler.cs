...
/// <summary>
/// Per-method profiler with both string and integer-ID APIs.
/// Integer IDs eliminate lookup overhead for hot-path instrumentation.
/// Call Register() from static field initializers, then use Begin(int)/End(int).
/// Emu thread calls Begin/End; UI thread reads Snapshot.
/// </summary>
static class Profiler
{
    ...
    /// <summary>
    /// Pre-register a named section and return its integer ID.
    /// Safe to call from static field initializers.
    /// </summary>
    ...
    public static void FrameEnd()
    {
        ...
        for (int i = 0; i < _count; i++)
        {
            ...
            stats[i] = new MethodStats
            {
                Name = a.Name,
                TotalMs = avgTotal,
                Calls = (long)Math.Round(avgCalls),
                AvgCallMs = avgPerCall,
                PeakMs = peak,
                PctFrame = pct,
            };
